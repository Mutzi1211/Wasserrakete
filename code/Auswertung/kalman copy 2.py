...
# Umschalt-Variable für Kalman-Filter
USE_KALMAN = False   
...
if USE_KALMAN:
    ...
    # Ohne Filter: einfach Originalspalten verwenden
    df["altitude_m_filtered"] = df["altitude_m"]
    df["accel_total"] = np.sqrt(
        df["accelX"]**2 +
        df["accelY"]**2 +
        df["accelZ"]**2
...
plt.title("Sensorverlauf {} Kalman-Filter".format("mit" if USE_KALMAN else "ohne"))
plt.tight_layout()
plt.show()
