...
#define PUBLISH_INTERVAL                                                       \
  1000 // Zeitintervall für die Veröffentlichung von Daten in Millisekunden
uint32_t nextMicros;
...
void show_display(String text) {
  ...
  if (!WiFi.status() == WL_CONNECTED && !recording) {
    ...
    if (showBattery)
      ...
      display.printf("Alt: \n %0.2fm", bme280.readFloatAltitudeMeters());
  }
  display.display();
}
...
bool check_parachute() {
  ...
  // if (bme280_active) {
  //   float static altitude = bme280.readFloatAltitudeMeters();

  //   Serial.println("Altitude: " + String(altitude) + "m");
  //   Serial.println("Last Altitude: " + String(last_altitude) + "m");
